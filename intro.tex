...
Unfortunately, instead of using flux, astronomers frequently characterize source brightness using the ancient-Greek-inspired system of magnitudes. The apparent (observed) magnitude of an object with flux $S$ is
\begin{equation}
m \equiv -2.5 {\rm log}_{10}(f/f_{\rm x}),
\end{equation}
where $f_{\rm x} = 2.53 \times 10^{-8} \ {\rm W}/{\rm m}^2$ is a reference flux \citep{RydenText}. Bright sources have smaller apparent magnitudes. \textcolor{red}{An absolute magnitude can be defined... necessary?} In terms of apparent magnitudes we can relate the lensed and intrinsic number densities using the equation
\begin{equation}
\label{eqn:nofm}
...
\textcolor{red}{size measurements more difficult than shapes, bottom pg 12 of Schneider ecopy. comparison pg 349 BS01}
